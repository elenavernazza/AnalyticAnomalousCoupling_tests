...
if __name__ == "__main__":
    ...
    plotting_nq["Combined"] = {
        "file_" : "/eos/user/g/gboldrin/varSens/SSWW_OSWW_OSWWQCD_WZeu_noQuad/results.txt",
        "models" : ["EFTNeg"],
        "Color" : ROOT.kGray+1,
    ...
    plotting["OSWW"] = {
        ...
        "file_" : "/eos/user/g/gboldrin/varSens/OSWW_OSWWQCD/sensPlots_OSWW_OSWWQCD/results.txt",
        "models" : ["EFTNeg"],
        "Color" : ROOT.kRed,
    }
    ...
    plotting["Combined"] = {
        ...
        "file_" : "/eos/user/g/gboldrin/varSens/SSWW_OSWW_WZeu_OSWWQCD/results.txt",
        "models" : ["EFTNeg"],
        "Color" : ROOT.kBlack,
    }
    ...
    frame = pad3.DrawFrame(min_x - 0.5*abs(max_x - min_x), min(y) - 0.2*abs(max(y) - min(y)), max_x + 1.8*abs(max_x - min_x), max(y) + 0.4*abs(max(y) - min(y)))
    ...
    for i,op in enumerate(first.keys()):
        ...
        for process in first[op]:
            ...
            tex_2s = ROOT.TLatex(max_x + 2.1*max_x, first[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(first[op][process]['2s'][0], first_nq[op][process]['2s'][0], first[op][process]['2s'][1], first_nq[op][process]['2s'][1]))
    ...
    frame = pad2.DrawFrame(min_x - 0.5*abs(max_x - min_x) , min(y) - 0.2*abs(max(y) - min(y)), max_x + 1.8*abs(max_x - min_x), max(y) + 0.2*abs(max(y) - min(y)))
    frame.GetXaxis().SetLabelFont( 63  )
    frame.GetXaxis().SetLabelSize(14)
    frame.GetYaxis().SetLabelSize(0)
    frame.GetYaxis().SetTickLength(0)
    ...
    for op in second.keys():
        ...
        for process in second[op]:
            ...
            if second[op][process]["graph_2s"].GetErrorXhigh(0) < max_x / 80 or second[op][process]["graph_2s"].GetErrorXlow(0) < min_x / 80: 
                name += " #times 25"
            ...
            if second[op][process]["graph_2s"].GetErrorXhigh(0) < max_x / 8 or second[op][process]["graph_2s"].GetErrorXlow(0) < min_x / 8: 
            ...
            if process == "WZ" and op in ['cHq1','cHWB','cHDD']:
               name = process + " #times 0.3"
               ...
               g1_f.SetFillColor(plotting_nq[process]["Color"])
               g1_f.SetFillColorAlpha(0, 1)
               g2.SetPointError(0, g2.GetErrorXlow(0)*0.3, g2.GetErrorXhigh(0)*0.3, g2.GetErrorYlow(0), g2.GetErrorYhigh(0))
               g1.SetPointError(0, g1.GetErrorXlow(0)*0.3, g1.GetErrorXhigh(0)*0.3, g1.GetErrorYlow(0), g1.GetErrorYhigh(0))
               g2_f.SetPointError(0, g2_f.GetErrorXlow(0)*0.3, g2_f.GetErrorXhigh(0)*0.3, g2_f.GetErrorYlow(0), g2_f.GetErrorYhigh(0))
               g1_f.SetPointError(0, g1_f.GetErrorXlow(0)*0.3, g1_f.GetErrorXhigh(0)*0.3, g1_f.GetErrorYlow(0), g1_f.GetErrorYhigh(0))
            ...
            tex_1s.SetTextFont(42)
            tex_1s.SetTextSize(0.019)
            tex_1s.SetLineWidth(2)

            tex_2s = ROOT.TLatex(max_x + 2.1*max_x, second[op][process]['gy'] ,"[{:.2f}({:.2f}),{:.2f}({:.2f})]".format(second[op][process]['2s'][0], second_nq[op][process]['2s'][0], second[op][process]['2s'][1], second_nq[op][process]['2s'][1]))
            tex_2s.SetTextFont(42)
            tex_2s.SetTextSize(0.019)
            tex_2s.SetLineWidth(2)
    ...
    tex4 = ROOT.TLatex(0.44,.01, "Parameter Estimate")
    tex4.SetNDC()
    ...
    tex5 = ROOT.TLatex(0.65,.84, "68% L+Q (L)")
    tex5.SetNDC()
    ...
    tex6 = ROOT.TLatex(0.835,.84, "95% L+Q (L)")
    tex6.SetNDC()
    ...
    c.Draw()
    c.Print("prova_SSWW_OSWW_OSWWQCD.pdf")
